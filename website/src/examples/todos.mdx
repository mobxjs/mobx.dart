...
As you can expect the **Todos** app manages a list of `Todo` items. Each `Todo` can be defined by a simple class like the following.
...
To render a useful UI, we also need a bunch of _derived (aka computed)_ properties. These properties are not inherent to the `TodoList` but derived from the core-state. Our UI looks like below, so you can identify the mapping of the computed properties to the various widgets.
...
Actions are the semantic operations that mutate the observable state. They also create transaction-boundary to ensure all notifications are sent out only at the end of an action. In our case, we have a few actions such as:
...
Co-location also helps in readability of your code and almost becomes self-explanatory. By choosing _Domain-specific_ names for your properties, actions and stores, there will be a clear mapping between the Business-domain and state code.

## Connecting the UI

Now that the state has been defined, the UI becomes a natural, and _visual_ extension of the Store. In case of MobX, we can sprinkle the _Widget-tree_ with as many `Observers` as needed. You can observe as little as you want or as much as needed!
