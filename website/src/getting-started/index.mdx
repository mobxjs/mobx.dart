...
import endResult from "./end-result.png";
...
Now, let's create a MobX store. The store allows us to use annotations and keeps the code simple. Create a new file `counter.dart` and add the following code to it.
...
- The generated code will be inside the part file: `counter.g.dart`. Without this, the `build_runner` will not produce any output.
...
Here's the output that we got from running it. Your's might be slightly different.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Getting Started with MobX'),
    );
  }
}
