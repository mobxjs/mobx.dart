...
description: Some tips and tricks for organizing Stores in MobX
---
...
Note that we are not making the `details` field into an `@observable` as its value is not going to change. The reactive-state that matters is inside the `PersonDetails`, which is already marked `@observable`.
  ...
  Widget build(BuildContext context) => MultiProvider(
          providers: [
            Provider<PreferencesService>(
              create: (_) => PreferencesService(_sharedPreferences),
            ),
