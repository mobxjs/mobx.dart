...
description: The conditions under which MobX will react and apply changes across your Stores and UI
---
...
This is the classic case of `Producer` **→** `Consumer` relationship. On one side is the observables, that notifies of changes and on the consumer side are reactions that _"react"_ to these changes. Without the **reactions**, MobX would be a a very quiet place with zero activity (reactivity?).
...
Since there are 3 forms of reactions: [`autorun`](/api/reaction#autorun), [`reaction`](/api/reaction#reaction), [`when`](/api/reaction#when), the tracking functions are slightly different.
