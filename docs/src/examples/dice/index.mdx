...
- Generate `dice_counter.g.dart` file using `build_runner` and this must be imported into the store. Read more about `build_runner` in the [MobX Code Generation library](https://github.com/mobxjs/mobx.dart/tree/master/mobx_codegen).
...
The working example will as seen in the figure below:
