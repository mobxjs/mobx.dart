...
import githubRepos from "../images/github-repos.gif";
...
Most of the widgets in the column should be self-explanatory. The simplest of these is the `LoadingIndicator`, a `StatelessWidget`, that tracks when the fetch operation is in progress.
...
The list-view for repositories also shows the state for an empty list of repos. If there is at least one repo, it will build the `ListView` and render the repositories. Notice the use of the computed filed `hasResults`, which simplifies the overall logic of the component.
