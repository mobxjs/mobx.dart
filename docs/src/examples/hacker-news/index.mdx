...
Rather than writing our own HTTP layer to talk to the hacker-news API, we will rely on a package: <PubBadge name="hnpwa_client" />, available on Pub. Here we will take a different perspective on defining the observable state.
