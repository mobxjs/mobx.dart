...
Thus for visual feedback, it is important to show each of these three states separately. `Pending` is normally shown with some loading-indicator. `Fulfilled` will be shown as as `ListView` of `FeedItem`s and finally _Rejected_ is the error state. You can see each of these in the following snippet:
