...
The `ConnectivityStore` is passed to the `ConnectivityExample` widget, which can be done through the `provider` package (refer to the documentation on [organizing stores](/guides/stores)).
