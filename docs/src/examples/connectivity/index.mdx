...
We'll need to use a `StatefulWidget` so that the reaction can be configured within the `initState` method and disposed of when the state itself is going to be disposed. The complete code is as follows
