...
We'll need to '`StatefulWidget` so that the reaction can be configured within the `initState` method and disposed of when the state itself is disposed. The complete code is as follows
