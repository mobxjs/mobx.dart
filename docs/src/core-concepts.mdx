...
For those who have experience with the JavaScript version of MobX, it's important to note that deep observability isn't possible with MobX for Dart as Dart is a statically typed language and there's no support for reflection in Flutter applications. This means with the Dart version of MobX, if a complex object is marked as observable then
MobX will not be able to automatically track changes to the object's properties. Should change tracking need to enabled on the object's properties then the properties need to be marked as observable as well.
