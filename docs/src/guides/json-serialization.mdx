...
The first step is to include the dependency on the [![pub package](https://img.shields.io/pub/v/dart_json_mapper.svg?label=dart_json_mapper&color=blue)](https://pub.dartlang.org/packages/dart_json_mapper) package. We add this to the `pubspec.yaml` and run `flutter packages get` to download it.
