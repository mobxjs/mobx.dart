...
- All the linked reactions (ones that depend on the observables mutated inside the action) are run only after the end of the action. This also ensures there are no pre-mature reactions occurring in the system.
...
As seen above, the raw form of using an `Action` can have a bit of boilerplate, especially as you start having lot of actions in your code. For most practical use-cases you are better off using the `Store` class and annotating functions with **`@action`**, as shown below:
...
The action methods can also be async, which allows you to intersperse mutations along with async calls. The code-generator takes care of ensuring all mutations are wrapped within actions.
