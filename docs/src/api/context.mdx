...
- **`never`**: Never enforce the read-policy. This allows reads to happen everywhere.
...
- **`observed`**: Enforce the mutation to happen inside an `Action`, only if the observable being mutated is being _observed_ in a `Reaction`. This is a middle ground between the `always` and `never` option. If the observable is not being observed, we allow the mutation and consider it safe.
