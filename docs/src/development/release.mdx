...
There is handy _npm-script_ called **`set:versions`** inside the root-level `package.json` file. You could run that to add the version to all packages in one go. Note that you do need [Node.js](https://nodejs.org) for this.
